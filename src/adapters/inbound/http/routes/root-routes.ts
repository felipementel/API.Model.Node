...
<html lang="pt-BR">
<head>
  <title>Usuarios API Node - Scalar</title>
  <meta charset="utf-8" />
  ...
  <div class="card">
    ...
    <div class="section">
      ...
      <ul>
        ...
        <li><a href="/docs/json">OpenAPI Spec <span class="badge">JSON</span></a></li>
      </ul>
    </div>
...
export const rootRoutes: FastifyPluginAsync = async (app) => {
  app.get('/', { schema: { hide: true } }, async (_, reply) => {
    reply.type('text/html').send(ROOT_HTML);
  });

  app.get('/scalar', { schema: { hide: true } }, async (_, reply) => {
